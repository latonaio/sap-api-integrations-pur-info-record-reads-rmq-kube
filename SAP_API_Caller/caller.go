...
import (
	...
	"io/ioutil"
	"net/http"
	sap_api_output_formatter "sap-api-integrations-pur-info-record-reads-rmq-kube/SAP_API_Output_Formatter"
	"strings"
	"sync"
...
func (c *SAPAPICaller) General(purchasingInfoRecord string) {
	...
	err = c.outputter.Send(c.outputQueues[0], map[string]interface{}{"message": generalData, "function": "PurchasingInfoRecordGeneral"})
	if err != nil {
		c.log.Error(err)
		return
	}
	...
	err = c.outputter.Send(c.outputQueues[0], map[string]interface{}{"message": orgPlantData, "function": "PurchasingInfoRecordOrganizationPlant"})
	if err != nil {
		c.log.Error(err)
		return
	}
	...
	err = c.outputter.Send(c.outputQueues[0], map[string]interface{}{"message": validityData, "function": "PurchasingInfoRecordPricingConditionValidity"})
	if err != nil {
		c.log.Error(err)
		return
	}
	...
	err = c.outputter.Send(c.outputQueues[0], map[string]interface{}{"message": cndnData, "function": "PurchasingInfoRecordPricingCondition"})
	if err != nil {
		c.log.Error(err)
		return
	}
...
func (c *SAPAPICaller) Material(purchasingInfoRecord, purchasingInfoRecordCategory, supplier, material, purchasingOrganization, plant string) {
	...
	err = c.outputter.Send(c.outputQueues[0], map[string]interface{}{"message": materialData, "function": "PurchasingInfoRecordOrganizationPlant"})
	if err != nil {
		c.log.Error(err)
		return
	}
...
func (c *SAPAPICaller) MaterialGroup(purchasingInfoRecord, purchasingInfoRecordCategory, supplier, materialGroup, purchasingOrganization, plant string) {
	...
	err = c.outputter.Send(c.outputQueues[0], map[string]interface{}{"message": materialGroupData, "function": "PurchasingInfoRecordOrganizationPlant"})
	if err != nil {
		c.log.Error(err)
		return
	}
